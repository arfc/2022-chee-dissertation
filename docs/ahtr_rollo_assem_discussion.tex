...
Table \ref{tab:a-2b-comparison-reactors} shows the $PF_{total}$, $PPF_{fuel}$, and TRISO 
packing fraction distribution variance for the following reactor models on Figure 
\ref{fig:assem-obj-2-pfppf-pareto}'s Pareto Front: reactor model 5 that 
minimizes both $PF_{total}$ and $PPF_{fuel}$ to an equal extent, reactor model 3 with 
most-minimized $PF_{total}$, and reactor model 1 with most-minimized $PPF_{fuel}$. 
\begin{table}[htbp!]
    \centering
    \onehalfspacing
    \caption{Flux value comparison between the two reactor models in Figure 
    ...
    \footnotesize
    \begin{tabular}{p{2cm}p{1cm}lll}
    \hline
    & \textbf{Reactor Model} & \textbf{$PF_{total}$} & \textbf{$PPF_{fuel}$} & \textbf{TRISO distribution variance} \\
    \hline 
    \textbf{Most Minimized Both} & 5 & 0.0583 & 1.1127 & 0.00044 \\
    \textbf{Most Minimized $PF_{total}$} & 3 & 0.0575 & 1.2056 & 0.00105 \\
    \textbf{Most Minimized $PPF_{fuel}$} & 1 & 0.0599 & 1.0906 & 0.00018 \\
    \hline
    \end{tabular}
\end{table}
