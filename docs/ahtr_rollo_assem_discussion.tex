...
$PF_{total}$ peaks in the 5th graphite plank (at 16.52cm) and has large $r_4$ and $r_5$
values. 

% coolant channel shape doesn't matter as much as TRISO distribution for temperature. 
...
from simulation a-2b and investigations in Section \ref{sec:assem-discussion-ppf-results} 
suggest that the minimize $PF_{total}$ objective's driving factor maximize total fission 
reaction rate and minimize $PPF_{fuel}$ objective's driving factor flattening thermal 
flux distribution influence each other resulting in unexpected TRISO distributions at 
