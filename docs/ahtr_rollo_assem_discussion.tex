...
fast neutrons born in the one-third assembly's center. 
The fast neutrons are moderated in the graphite matrix and graphite structure 
(\gls{AHTR} one-third assembly geometry: Figure \ref{fig:ahtr_assembly}). 
The moderated neutrons are more likely absorbed in the fuel regions nearer to 
...
\gls{ROLLO} successfully found widely spread out reactor model solutions in each  
...
These reactor models on the Pareto Front have different TRISO distributions and coolant
channel shapes, depending on the extent each objective is minimized due to the nature 
of multi-objective optimization that results in a tradeoff between objectives. 
...
all the objectives. 
\gls{ROLLO} also provides the reactor designer with a sense of how sensitive 
each input parameter is in relation to the objectives. 
Once the \gls{ROLLO} search is complete, reactor designers have access to the narrower 
reactor design space that meets their defined objectives and gain a better intuition of 
the model's reactor physics. 
...
further study a smaller section of the optimal design space. 
