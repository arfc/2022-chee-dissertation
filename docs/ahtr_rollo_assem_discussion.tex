...
This relationship between $PF_{total}$ and $PPF_{fuel}$ for the \gls{AHTR} geometry 
...
most-minimized $PF_{total}$ objective, reactor model xx (Figure 
\ref{fig:assem-obj-3-most-minimized-distr}), has an oscillating TRISO distribution 
along the both the x-axis and y-axis. 
Figure \ref{fig:a-3a-pf-triso-comparison} compares simulation a-3a's reactor model xx and 
simulation a-1a's most-minimized $PF_{total}$ reactor model. 
\begin{figure}[htbp!]
    \centering
...
In simulation a-3a, \gls{ROLLO} found that the one-third assembly model with the 
most-minimized $T_{max}$ objective, reactor model xx (Figure 
...
$T_{max}$ reactor model xx and simulation a-1b's most-minimized $T_{max}$ reactor model. 
\begin{figure}[htbp!]
    \centering
...
\end{figure}
Figure \ref{fig:a-3a-temp-triso-comparison} shows that simulation a-2a's most-minimized 
$T_{max}$ reactor model and simulation a-1b's most-minimized $T_{max}$ reactor model 
have similar almost constant TRISO distributions with packing fraction standard 
deviations of $0.003$ and $0.0009$, respectively.
However, they have different $PF_{total}$ values. 
