...
software (DEAP), \gls{ROLLO}'s input file structure, and details about \gls{ROLLO}'s 
software architecture, methods used to verify \gls{ROLLO}, and \gls{ROLLO}'s stopping 
criteria. 
...
\begin{table}[]
    ...
    \end{tabular}
    \end{table}
%%% UPDATE THIS: perhaps put hyperparameter studies in appendix? 
...
The second parallel mode, \texttt{job\_control}, utilizes unix systems job control 
...
Many nuclear softwares, such as OpenMC and MOOSE, use \gls{MPI} or OpenMP to parallelize their runs. 
...
The first layer being the individual software runs' parallelization, and the second layer being ROLLO's parallelization. 
For example, running four reactor models using ROLLO across 16 nodes on a cluster, and assigning each reactor model to run
in parallel across four nodes. 
In-depth details about parallelization setup can be found in the \gls{ROLLO} documentation
...
\begin{align}
    ...
    \end{cases}
\end{align}
We use it as a performance test for multi-objective optimization.
