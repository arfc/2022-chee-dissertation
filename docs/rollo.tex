...
I added an integration test to \gls{ROLLO} that checks that a default \gls{ROLLO} simulation 
will find the Ackley function's global optimum point. 
\gls{ROLLO} successfully passes it. 


\subsection{Pu239 Critical Bare Sphere}
To ensure \gls{ROLLO} works correctly while coupled with OpenMC, I run \gls{ROLLO} 
to find the critical radius for a Plutonium-239 bare sphere. 
...
Figure \ref{fig:verification_sphere} shows the ROLLO input file for this verification 
problem. 
\begin{figure}[]
    \begin{minted}[
        frame=lines,
