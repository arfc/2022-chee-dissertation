...
For each evaluator, mandatory input parameters are \texttt{order}, \texttt{input\_script}, 
\texttt{inputs}, and \texttt{outputs}, and the optional input parameters are
\texttt{execute} and \texttt{output\_script}. 
Table \ref{tab:evaluator-inputs} describes the input parameters accepted for each evaluator in 
ROLLO's \texttt{evaluators} input file section.  
\begin{table}[]
    \centering
    \onehalfspacing
    \caption{}
    ...
    \hline
    \multirow{6}{2cm}{Mandatory Parameters} & \texttt{order} & int 
    & evaluator's operational order compared to other evaluators (indexed by 0) \\
    \cline{2-4}
    & \texttt{inputs} & list of strings & control variables to be placed into the input file template \\
    \cline{2-4}
    & \texttt{input\_script} & list of strings (2-element) & first element: executable to run input script, 
    second element: input file template's name \\
