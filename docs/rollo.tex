...
For each \gls{ROLLO} simulation, nuclear reactor modeling software must be run to 
calculate the fitness values for each reactor model in each generation; i.e. a \gls{ROLLO} 
simulation that runs for 5 generations with 100 reactor models will require 500 separate 
reactor modeling software runs. 
Computing fitness values with nuclear reactor modeling software is computationally 
expensive, necessitating efficient software parallelization on supercomputers. 
