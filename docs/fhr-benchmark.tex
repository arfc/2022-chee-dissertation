...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.32\linewidth]{p1a_c1a_e_eg1.png} 
    \includegraphics[width=0.32\linewidth]{p1a_c1a_e_eg2.png} 
    \includegraphics[width=0.32\linewidth]{p1a_c1a_e_eg3.png} 
    \includegraphics[width=0.32\linewidth]{p1a_3a_e_eg1.png} 
    \includegraphics[width=0.32\linewidth]{p1a_3a_e_eg2.png} 
    \includegraphics[width=0.32\linewidth]{p1a_3a_e_eg3.png} 
    \includegraphics[width=0.32\linewidth]{p1a_6a_e_eg1.png} 
    \includegraphics[width=0.32\linewidth]{p1a_6a_e_eg2.png} 
    \includegraphics[width=0.32\linewidth]{p1a_6a_e_eg3.png} 
    \caption{\gls{UIUC} \gls{FHR} benchmark flux results: neutron flux 
    distribution in 100 $\times$ 100 mesh for three coarse energy groups normalized 
    by each group's average flux.
    Three cases are shown: Case 1A (above), Case 3A (middle), Case 6A (below). 
    Energy group 1: $E > 0.1$ MeV, 
    Energy group 2: $3 \times 10^{-6} < E < 0.1$ MeV, 
    Energy group 3: $E < 3 \times 10^{-6}$ MeV. }
    \label{fig:phase1a-e}
\end{figure} 
...
\begin{figure}[!htb]
    ...
    \includegraphics[width=0.49\linewidth]{p1a_6a_f.png} 
...
\subsubsection{Key Neutronics Parameters Verification}
...
\subsection{Moltres Mesh Generation}
To run Moltres simulations, the user must provide a mesh file representing the reactor 
geometry. 
...
\begin{figure}[!htb]
    ...
    I meshed the geometry file using Gmsh \cite{geuzaine_gmsh_2009}.
...
I performed a mesh refinement study on the Moltres \gls{AHTR} full assembly temperature 
...
The mesh refinement study shows $||\Delta T_k||_N$ convergence with more 
refinement steps. 
I used x4 mesh refinement for the Moltres \gls{AHTR} full assembly temperature model
because $||\Delta T_k||_N = 0.053K$, which I evaluated as sufficiently converged.  

\subsection{Temperature Model Setup and Results}
...
The Moltres input file is set up using the assumptions and constants described in the 
previous subsection. 
...
\subsubsection{Temperature Model Setup}
...
equation (Equation \ref{eq:moltres-temp}) as I am running a steady-state model with
...
AHTR models generate constant amount of power, which is all removed by the coolant, and 
that there is linear increase in temperature from inlet to outlet. 
\begin{align}
      \label{eq:calc-htc-full-assem}
      h_{assem} &= \frac{P_{dz}}{V_{coolant \cdot \Delta T}} \\
      &= \frac{26223 W/cm}{471cm^3 \cdot 0.0909 K\cot cm^{-1}} \nonumber \\
      &= 611 W cm^{-3}K^{-1} \nonumber 
\intertext{and}
\Delta T  &= \frac{T_{total}}{H} = \frac{50 K}{550 cm} = 0.0909 K/cm
\intertext{where}
h_{assem} &= \mbox{assembly's heat transfer coefficient } [W cm^{-3} K^{-1}] \nonumber \\
P_{dz} &= \mbox{power produced in 1cm AHTR assembly $\Delta z$ slice [$W \cdot cm^{-1}$]}\nonumber \\
...
\subsubsection{Temperature Model Results}
