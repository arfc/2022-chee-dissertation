...
\begin{align}
    ||T_{diff}||_N &= \frac{1}{N}\sqrt{\sum_{i=1}^N(T_{all, i} - T_{i})^2} \\
\intertext{where}
    ||T_{diff}||_N &= \mbox{normalized 2-norm temperature difference} \nonumber \\
...
\begin{table}[htbp]
    ...
    \textbf{Max Slab Temp Diff [K]} & $||T_{diff}||_N$ \\ 
    \hline 
