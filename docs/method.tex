...
In the following sub-sections, I describe the \gls{AHTR} modeling workflow: the varied
\gls{AHTR} input parameters, the OpenMC and Moltres models, and the output 
and constraint values calculation. 
...
\begin{table}[htbp]
    ...
    of discretized temperature difference across the slab's centerline for varying 
...
I defined the \gls{AHTR} input parameters varied for the optimization problems: 
\acrfull{TRISO} packing distribution, total fuel packing fraction and coolant 
channel shape. 
I also defined the key optimization objectives: minimize fuel amount, maximize 
heat transfer, and minimize power peaking.
I described the \gls{AHTR} plank and one-third assembly geometries. 
I detailed the \gls{AHTR} modeling workflow: the control variables used to vary 
...
I conducted a key neutronics parameter verification which ensured that the spatial and
energy homogenizations used for the plank and one-third assembly Moltres temperature 
models preserved accuracy.
I described the \gls{ROLLO} hyperparameter tuning process and reported the final 
hyperparameters used for the single and multi-objective optimization problems. 

The next two chapters: \gls{AHTR} plank optimization results and \gls{AHTR} 
one-third assembly optimization results use the methods described in this chapter. 