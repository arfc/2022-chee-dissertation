...
%TODO describe why I chose maximum temperature, rather than average etc. 
...
The mesh refinement study reports the average, maximum, and normalized 2-norm temperature 
difference between refinement steps (Equation \ref{eq:2-norm-tdiff-i}).
\begin{align}
...
models. 
Further refinement is unfeasible due to the large mesh size resulting in insufficient memory 
on supercomputers to save the mesh for Moltres runs. 
...
\intertext{where}
    ...
    N &= \mbox{total number of discretized points} \nonumber \\
    T_{all} &= \mbox{temperature from model with group constant data with all four temperatures} \nonumber \\
    T &= \mbox{temperature from model with group constant data with one temperature} \nonumber 
\end{align}
