...
The \gls{AHTR} modeling workflow falls within the \textit{evaluate population} orange blocks 
in Figure \ref{fig:genetic_alg_nuclear}'s genetic algorithm flow chart.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[node distance=0.5cm]
        ...
        \node (9) [e82block, above=of 8] {Nuclear Software Models};
...
For spatial homogenization of the straightened \gls{AHTR} fuel plank and 
...
\begin{figure}[htbp]
    ...
    \resizebox{0.5\textwidth}{!}{
        \hspace{1cm}
        \fbox{\begin{tabular}{ll}
            \textcolor{fhrblue1}{$\blacksquare$} & inter-assembly FLiBe \\
            \textcolor{fhrgrey1}{$\blacksquare$} & Y-shaped graphite structure \\
            \textcolor{fhrblue2}{$\blacksquare$} & control rod slot FLiBe \\
            \textcolor{fhrblue3}{$\blacksquare$} & inter-plank FLiBe \\
    ...
    70 \textit{cells} for OpenMC multigroup calculation.}
...
spatial homogenizations outlined in Section \ref{sec:ahtr-moltres-hom}.
...
The Moltres \gls{AHTR} Steady-State Temperature Model is a 2D model of the \gls{AHTR} plank and 
one-third assembly x-y cross-section with geometry from 
Figure \ref{fig:straightened_slab_mg} and Figure \ref{fig:fhr_onethird_assem_homogenization}
, respectively.  
% 3d model with unit thickness 
The model first solves the \gls{AHTR} neutronics and uses that to solve the \gls{AHTR}'s 
...
\begin{table}[htbp]
    \centering
    \onehalfspacing
    \caption{\acrfull{AHTR} materials' thermal conductivity.}
...
\begin{table}[htbp]
    ...
    \hline 
    Plank & $h_{plank}$ & 990 & $\frac{W}{cm \cdot K}$ & Calculated in Eq. \ref{eq:calc-htc} \\
    Assembly & $h_{assem}$ & 611 & $\frac{W}{cm \cdot K}$ & Calculated in Eq. \ref{eq:calc-htc} \\
    Both & $T_{ref}$ & 923 & K & AHTR Inlet Temperature \\ %cite ahtr 923K inlet temp 
    \hline
    \end{tabular}
\end{table} 
...
The AHTR plank model with TRISO-level fidelity is depicted in Figure \ref{fig:ahtr-plank-verification}. 
 \begin{figure}[htbp]
    \centering
...
The $k_{eff}$ are acceptable since I am not using the Moltres simulation for 
...
Table \ref{tab:ahtr_plank_moltres_coeffs} shows that the $\beta{eff}$ values from OpenMC and 
Moltres show excellent agreement with a discrepancy of 0.1pcm. 
...
Table \ref{tab:ahtr_plank_moltres_coeffs} shows that the total temperature coefficients from 
OpenMC and Moltres show good agreement with a discrepancy of 0.4 pcm/K.
\begin{table}[htbp]
    \centering
    \onehalfspacing
...
The AHTR one-third assembly model with TRISO-level fidelity is depicted in Figure 
...
\ref{tab:betaeff_ahtr_moltres_assem} show excellent agreement with a discrepancy of 0.8pcm. 
I calculated the temperature reactivity coefficients with Equation 
\ref{eq:reactivity-coefficient}.
Temperature reactivity feedback arises mainly from Doppler broadening of 
resonance absorption peaks and thermal expansion.
Table \ref{tab:reactivity_ahtr_moltres_assem} shows that the total temperature 
coefficients from OpenMC and Moltres show good agreement with a discrepancy of 0.2 pcm/K.
\begin{table}[htbp]
    \centering
    \onehalfspacing
...
Figure \ref{fig:assem_neutron_spectrum_948K} shows the neutron spectrum of the OpenMC 
...
I performed a mesh refinement studies on the \gls{AHTR} plank and one-third assembly 
...
\begin{table}[htbp]
    ...
    \hline 
    1 & 1184.43 & - & 995.96 & -\\
    2 & 1186.04 & +0.61 & 1001.75 & +5.79\\
    3 & 1185.99 & -0.05 & 1003.62 & +1.87\\ 
    4 & 1187.65 & +1.66 & 1004.71 & +1.09\\
    \hline
    \end{tabular}
\end{table}
...
Tables \ref{tab:moltres-group-constant-temps} and \ref{tab:moltres-group-constant-temps-assem}
shows the average and maximum temperature reported by each Moltres model and their 
differences compared to the model with group constant data with all four temperatures for 
the plank and one-third assembly models, respectively. 
Both tables also shows the 2-norm of the centerline temperature difference between the 
AHTR model with all four temperatures and each single temperature.
...
\begin{table}[htbp]
    ...
    \textbf{Diff [K]} & $||T_{all, i} - T_{i}||$ \\ 
    \hline 
    All  & 1004.366 &  -     & 1186.359 & -      & -    \\
    948 & 1004.708 & +0.341 & 1187.658 & +1.299 & 8.876 \\
    1024 & 1004.676 & +0.310 & 1187.148 & +0.788 & 8.178 \\
    1100 & 1004.622 & +0.256 & 1186.085 & -0.274 & 9.119\\
    1200 & 1004.539 & +0.172 & 1185.359 & -1.000 & 9.398 \\
    \hline
    \end{tabular}
\end{table}
...
The 1024K single temperature group constant data has the lowest 2-norm, thus, I 
