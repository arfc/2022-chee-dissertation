...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[node distance=0.5cm]
        ...
        \node (3) [e82block, above=of 2] {Independent Variables};
...
Since the same materials, uranium isotope, and general plank model 
structure is used for the \gls{AHTR} plank's optimization, conducting 
...
Comparison of $k_{eff}$ and reactivity verify that the Moltres model is replicating 
...
%TODO: ask SM
% are there any potential risks for this? Statae them here. Could it be possible 
% that you are over or under estimating heat produced? 

\subsubsection{AHTR Plank: Reactivity Coefficients}
Comparison of reactivity coefficients produced by Moltres and OpenMC verify that 
...
Comparison of flux distributions produced by Moltres and OpenMC verify that 
the Moltres model is replicating the OpenMC model's neutronics correctly, and are also 
important to ensure that Moltres accurately calculates the temperature distribution.
...
% Max is siginificant but not surprising for neutron diffusion. 
% Moltres is able to reproduce the correct trend. 
% This level of accuracy is sufficient for comparing max temp. 

\subsubsection{AHTR Plank: Neutron Energy Spectrum}
Comparison of the neutron energy spectrum verifies that the Moltres model is replicating 
the OpenMC model's neutronics correctly.
...
and Moltres models, good agreement in their reactivity coefficients, and 4-group neutron 
energy spectrum.
...
The differences in reactivity and flux at specific points might result in slightly 
...
different \gls{AHTR} models are captured correctly, \gls{ROLLO} is able to accurately 
find optimal reactor geometries. 
...
%Even though the reactivity and $k_{eff}$ values different, the reactivity coefficients 
%are in agreement, therefore, Moltres can capture the relative temperature 


%the neutron diffusion method used in Moltres does not 
%approximate this reactor geometry as well. 
%The diffusion coefficients for this reactor type are approximately 1cm.
%The \gls{FLiBe} coolant channel has a 0.35cm width which is smaller than the diffusion
%coefficient, resulting in poor approximations. 
...
% TODO: If you expect any deviations, state them here and say why/why not.
% primarily from using neutron diffusion instead of neutron transport methods 
% flux not being reproduced in small regions which are less than a few mean free 
% path if neutrons, near material interfaces. 
% Even though the keff differ, the Beta eff values agree and so the Moltres model 
% can capture the relative temperature distribution accurately. 
% baseline is off, but the relative values are accurate. The  
% normalized power, keff is ok. normalizing power = normalizing the number of fission 
% neutrons produced. 

% using the same materials, the same uranium isotope, the same general structure, 
% these 3 contribute to similar neutron energy spectrum. so should be ok. 

\subsection{AHTR One-Third Assembly: Key Neutronics Parameters Verification}
\label{sec:assem-knp}
Key neutronics parameter verificaiton is necessary for the one-third assembly model 
as well since it has different ratios of materials compared to the plank model.
...
% TODO: for all of these, add description on how plank and 1/3 assem are similar or 
%different. 
...
\begin{table}[htbp]
    ...
    \end{tabular}
\end{table}
% are these beff what you would expect for this fuel type? How does it change with 
% burnup? 
% What do these differences apply about Monte Carlo vs diffusion. Can we systematically 
% assume this difference will be similar? 
...
% TODO: why can't we see the 11% diff in the plots. add explanation. 
...
% TODO: are these accurate? Say how much those 11% differences may affect the source 
%term. connect it for readers. 
...
The one-third assembly verification study found a $381$ pcm reactivity difference 
between OpenMC and Moltres models, good agreement in their reactivity coefficients, 
and 4-group neutron energy spectrum.
The verification study also found good agreement in overall flux distributions 
(based on the 2-norm difference), however there were larger flux differences at 
specific points. 
The reactivity and flux differences are due to Moltres utilizing the neutron diffusion 
...
converged. 
% TODO: how do you know when they are converged? What criteria? What happens if they 
% are not converged? 
% good enough. 
% 2 norm, N = 2
...
% TODO: So does this mean you aren't converged or that future work needs to split the 
% problem somehow... How do we know this is good enough? 
...
% moltres does its own interpolation based on the group constants from OpenMC. 
% i'm doing this to verify but I don't expect it to matter... worth checking to see the 
% effect. 
...
% TODO: make sure to note here how OpenMC treats temperature for XS. Why do you 
% expect to see differences? Does Temp interpolation of resonances matter here? 
% TODO: just describe better.. 
...
generate the group constant data for the Moltres temperature models.
% TODO: What do these small differences mean for modeling more generally? That is, 
%do XS need to be regenerated frequently? OR can you use the same ones often. 
...
\begin{table}[htbp]
    ...
    \cline{2-6}
    \textbf{Independent/Output Variables} & \textbf{6} & \textbf{15} & \textbf{24} & \textbf{36} & \textbf{39}\\
    \hline 
...
% TODO: your explanation about compute time is good here, but you might want to 
% comment on how different results for hyperparmateres might be with OpenMC 
% in this multi-obj domain. 
