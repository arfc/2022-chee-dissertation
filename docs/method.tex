...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[node distance=0.5cm]
        ...
        \node (2) [b72block, right=of 1] {$PF_{total}$};
        ...
        \node (16) [b72block, below=of 15] {$PPF_{fuel}$};
...
An \gls{AHTR} plank with $PF_{total} = 0.0979$, and packing fraction 
distribution of 
...
\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=\linewidth]{triso_distribution_sine.png}} 
...
An \gls{AHTR} one-third assembly with $PF_{total} = 0.1$ and packing 
fraction distribution of 
...
The Moltres temperature model first solves the \gls{AHTR} neutronics and uses the outputs 
to solve the \gls{AHTR}'s temperature distribution for a defined power.
...
However, an in-depth \gls{AHTR} Moltres model that includes turbulence modeling is 
...
\intertext{where} 
Q_f &= \mbox{source term } [MeVcm^{-3}s^{-1}] \nonumber \\
...
\intertext{where}
Q &= \mbox{heat removal rate for 1cm thin slice of the AHTR model } [W cm^{-1}] \nonumber \\
h &= \mbox{heat transfer coefficient } [W cm^{-3} K^{-1}] \nonumber \\
...
\begin{table}[htbp]
    ...
    \hline 
    Plank & $h_{plank}$ & 990 & $W cm^{-3} K^{-1}$ & Calculated in Eq. \ref{eq:calc-htc-plank} \\
    One-Third Assembly & $h_{assem}$ & 611 & $W cm^{-3} K^{-1}$ & Calculated in Eq. \ref{eq:calc-htc-assem} \\
...
\begin{align}
    ...
    h_{plank} &= \frac{P_{dz}}{V_{coolant}} \div \Delta T \\
      &= \frac{1456 W cm^{-1}}{23.1cm \times 0.35cm \times 1cm \times 2} \div 0.0909 K cm^{-1} \nonumber \\
      &= 990 W cm^{-3}K^{-1} \nonumber \\
      \label{eq:calc-htc-assem}
      h_{assem} &= \frac{P_{dz}}{V_{coolant}} \div \Delta T \\
      &= \frac{8741 W cm^{-1}}{157cm^3} \div 0.0909 K cm^{-1} \nonumber \\
      &= 611 W cm^{-3}K^{-1} \nonumber 
\intertext{and}
\Delta T  &= \frac{T_{total}}{H} = \frac{50 K}{550 cm} = 0.0909 K cm^{-1}
\intertext{where}
h_{plank} &= \mbox{plank's heat transfer coefficient } [W cm^{-3} K^{-1}] \nonumber \\
h_{assem} &= \mbox{one-third assembly's heat transfer coefficient } [W cm^{-3} K^{-1}] \nonumber \\
P_{dz} &= \mbox{power produced in 1cm AHTR plank/one-third assem $\Delta z$ slice } [W cm^{-1}] \nonumber \\
...
For this verification study, I used an \gls{AHTR} plank model with a 0.0979 total packing 
