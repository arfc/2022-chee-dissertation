...
\begin{table}[htbp]
    \centering
    \onehalfspacing
    \caption{AHTR one-third assembly's $\beta_{eff}$ values from OpenMC and Moltres at 948K.}
	\label{tab:betaeff_ahtr_moltres_assem}
    \footnotesize
    \begin{tabular}{lllc}
    \hline 
    \textbf{Software}& \textbf{Homogenized?}& \textbf{$\beta_{eff}$ [pcm]} 
...
\begin{table}[htbp]
    \centering
    \onehalfspacing
    \caption{AHTR one-third assembly's total reactivity coefficient values calculated from 
    OpenMC and Moltres at 948K and 1100K.}
	\label{tab:reactivity_ahtr_moltres_assem}
    \footnotesize
    \begin{tabular}{lllc}
    \hline 
    \textbf{Software}& \textbf{Homogenized?}& \textbf{Total $\frac{\Delta \rho}{\Delta T}$ [pcm/K]} 
...
To ensure that the geometry mesh input to the Moltres temperature model is sufficiently converged, 
I performed a mesh refinement study, outlined in Table \ref{tab:ahtr-plank-mesh-refinement}. 
\begin{table}[htbp]
    \centering
    \onehalfspacing
    \caption{AHTR plank's mesh refinement study.}
	\label{tab:ahtr-plank-mesh-refinement}
    \scriptsize
    \begin{tabular}{lll}
    \hline 
    \textbf{Refinement} & \textbf{Max Plank Temp [K]} 
    & \textbf{Max Plank Temp [K] Diff} \\ 
    \hline 
    1 & 1126.219 & - \\
    2 & 1127.711 & -1.491 \\
    3 & 1128.434 & -0.723 \\ 
