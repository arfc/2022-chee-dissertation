...
specific points (based on the maximum difference). 
The reactivity and flux differences are due to Moltres utilizing neutron diffusion 
...
The differences in reactivity and flux at specfic points might result in slightly 
...
agreement, Moltres can accurately capture the relative temperature distributions. 
During the \gls{AHTR} optimization, so long the relative maximum temperature between 
different \gls{AHTR} models are captured, \gls{ROLLO} is able to accurately find 
optimal reactor geometries. 
Reactor designers can then use higher fidelity software for the final optimal 
\gls{AHTR} geometry modeling. 
...
% TODO: say why you chose this and why there isn't a need to change it. 
