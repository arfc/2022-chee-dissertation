...
\begin{align}
    ...
    \cdot \nabla T + \nabla \cdot \left(k \nabla T \right) + Q_f
    \intertext{and}
    Q_f l \sum^G_{g=1} \epsilon_g \Sigma_g^f \phi_g
    \intertext{where}
...
\begin{figure}[btp]
    ...
    \begin{subfigure}{0.49\textwidth}
        ...
        \label{fig:see2} 
    \end{subfigure}
    \caption{Initial design versus the optimal design of outlet plenum, reproduced 
    from \cite{see_design_2022}. The optimal design meets the following design objectives: 
    0.5-psi pressure drop across the outlet plenum, monitor core coolant flow average 
    temperature within $\pm 5 ^{\circ}C$.}
...
\begin{figure}[btp]
    ...
    plenum, reproduced from \cite{see_design_2022}.The optimal design meets the following 
    design objectives: 0.5-psi pressure drop across the outlet plenum, monitor core 
    coolant flow average temperature within $\pm 5 ^{\circ}C$.}
    \label{fig:see3}
\end{figure}
...
Selection operator methods utilized in this dissertation include tournament 
