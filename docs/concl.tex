...
All the single and two objective optimizations led to the final largest optimization 
problem: the one-third assembly multi-objective optimization that ran for 8 generations 
with 256 reactor models per generation, also known as simulation a-3b-256. 
Simulation a-3b-256 minimized all three objectives ($PF_{total}$, $T_{max}$, and 
$PPF_{fuel}$) while varying all the input parameters ($PF_{total}$, TRISO 
distribution, and coolant channel shape). 
...
geometries and parameters, and to optimize for objectives that address the many faceted 
multiphysics interactions and feedbacks present in the reactors. 
