...
The results from the multi-objective optimization simulations suggested that 
the minimize $PF_{total}$ objective's driving factor maximize total fission reaction
rate and minimize $PPF_{fuel}$ objective's driving factor flattening thermal flux 
distribution influence each other resulting in unexpected TRISO distributions at 
different $PF_{total}$ values.
Characterizations of each objective for the simple \gls{AHTR} plank model and 
understanding the relationship between the minimize $PF_{total}$ and $PPF_{fuel}$ 
objectives provide insights to better understand Chapter 
\ref{chap:ahtr-assem-opt-results}'s multi-objective optimization of the complex 
\gls{AHTR} one-third assembly model.   
% TODO: add a few sentences on key takeaways here too! Comment on anything surprising 
% that you found. 
