...
Figure \ref{fig:assem-obj-2-pfppf-most-minimized} shows the two distributions from 
Figure \ref{fig:slab-obj-3-distr} that most-minimized $PF_{total}$ and $PPF_{fuel}$. 
\begin{figure}[htbp!]
    \centering
    \begin{subfigure}{\textwidth}
    ...
    \end{subfigure}
    \caption{AHTR one-third models and TRISO distribution for the 3 reactor models on 
    simulation p-2b's Pareto front that most-minimized each objective.
    Simulation p-2b -- ROLLO two-objective optimization to minimize total fuel 
    packing fraction ($PF_{total}$) and normalized power peaking factor ($PPF_{fuel}$) 
    in the \gls{AHTR} one-third assembly. 
