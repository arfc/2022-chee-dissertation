...
\begin{table}[htbp!]
    ...
    \footnotesize
    \begin{tabular}{p{1.5cm}|l|lllp{3cm}}
    \hline 
    \textbf{Objs [\#]} & \textbf{Sim} & \textbf{Objectives} & \textbf{Constraints} &\textbf{Varying Parameters} & \textbf{Total Compute Cost [node-hours]} \\
    \hline
    \multirow{9}{2cm}{1}& a-1a & \tabitem min($PF_{total}$) & \tabitem $k_{eff} \geq 1.38$ &\tabitem $\rho_{TRISO}(\vec{r})$ & 285.8 \\
    & & & & \tabitem $PF_{total}$ & \\
    \cline{2-6}
    & a-1b & \tabitem min($T_{max}$) & \tabitem $k_{eff} \geq 1.38$ &\tabitem $\rho_{TRISO}(\vec{r})$ & 740.9\\
    \cline{2-6}
    & a-1c & \tabitem min($PPF_{fuel}$) & \tabitem $k_{eff} \geq 1.38$ &\tabitem $\rho_{TRISO}(\vec{r})$ & 230.0\\
    \cline{2-6}
    & a-1d & \tabitem min($PF_{total}$) & \tabitem $k_{eff} \geq 1.0$ &\tabitem Coolant channel shape & 334.6 \\
    & & & & \tabitem $PF_{total}$ & \\
    \cline{2-6}
    & a-1e & \tabitem min($T_{max}$) & \tabitem $k_{eff} \geq 1.38$ &\tabitem Coolant channel shape & 692.3\\
    \cline{2-6}
    & a-1f & \tabitem min($PPF_{fuel}$) & \tabitem $k_{eff} \geq 1.0$ &\tabitem Coolant channel shape & 222.9\\
    \hline
    \multirow{6}{2cm}{2}& a-2a & \tabitem min($PF_{total}$) & \tabitem $k_{eff} \geq 1.38$ & \tabitem $\rho_{TRISO}(\vec{r})$ & 1250.9\\
    & &\tabitem min($T_{max}$) & & \tabitem $PF_{total}$ & \\
    \cline{2-6}
    & a-2b & \tabitem min($PF_{total}$) & \tabitem $k_{eff} \geq 1.38$ & \tabitem $\rho_{TRISO}(\vec{r})$ & 491.7\\
    & & \tabitem min($PPF_{fuel}$) & & \tabitem $PF_{total}$ & \\
    \cline{2-6}
    & a-2c & \tabitem min($T_{max}$) & \tabitem $k_{eff} \geq 1.38$ & \tabitem $\rho_{TRISO}(\vec{r})$ & 537.7\\
    & & \tabitem min($PPF_{fuel}$) & & & \\
    \hline
    \multirow{6}{2cm}{3}& a-3a &\tabitem min($PF_{total}$) & \tabitem $k_{eff} \geq 1.38$ & \tabitem $\rho_{TRISO}(\vec{r})$ & 1367.9\\
    && \tabitem min($PPF_{fuel}$) & & \tabitem $PF_{total}$ & \\
    && \tabitem min($T_{max}$) & & & \\
    \cline{2-6}
    & a-3b &\tabitem min($PF_{total}$) & \tabitem $k_{eff} \geq 1.38$ & \tabitem $\rho_{TRISO}(\vec{r})$ & 1830.0\\
    && \tabitem min($PPF_{fuel}$) & & \tabitem $PF_{total}$ & \\
...
Figure \ref{fig:assem-obj-1-pf-evol} shows simulation a-1a's $PF_{total}$ evolution.
...
Figure \ref{fig:a-1d} shows the plots of the simulation a-1d final generation 
...
one-third assembly. 
Because it is more productive to compare all of the single objective results to one 
another, Section \ref{sec:assem-discussion-single} discusses and explains simulation 
...
This section describes the single-objective a-1c and a-1f optimization simulation
results. 
...
Section \ref{sec:assem-discussion-single} discusses the driving factors for the minimize 
...
Section \ref{sec:assem-discussion-single} concluded that for the \gls{AHTR} plank model, 
...
660pcm higher $k_{eff}$ than the constant TRISO distribution due to the higher total 
...
$T_{max}$. 
Figures \ref{fig:assem-obj-1-temp-evol} and \ref{fig:a-1e-evol} show 
that during the genetic algorithm optimization process, the average $T_{max}$ due to 
\gls{TRISO} variation decreased by 150K, while average $T_{max}$ due to the coolant 
channel shape variation only decreased by 5K, demonstrating that TRISO distribution 
influences the minimize $T_{max}$ objective more than the coolant channel shape. 
...
Section \ref{sec:assem-discussion-single} concluded that for the \gls{AHTR} plank 
...
\begin{figure}[htbp!]
    ...
    \caption{AHTR one-third assembly's flux comparison in units of 
    $n \cdot cm^{-2} \cdot s^{-1}$ between the two 
    reactor models in Figure \ref{fig:a-1c-ppf-triso-comparison}: 
    simulation a-1c's most-minimized 
...
Section \ref{sec:assem-discussion-two} discusses and explains simulation a-2a's 
results.
...
$PF\approx0.035$. 
Sections \ref{sec:assem-discussion-two} discusses and explains simulation a-2b's 
results.
...
Section \ref{sec:assem-discussion-two} discusses and explains simulation a-2c's 
results.
...
Section \ref{sec:assem-discussion-single} described both influences in detail. 
...
Section \ref{sec:assem-discussion-single} described that in the \gls{AHTR} plank model, 
...
conduct an in-depth examination into the driving factors for the minimize 
$PF_{total}$ and minimize $PPF_{fuel}$ objectives. 

\paragraph{Driving Factor Quantification}
Section \ref{sec:assem-discussion-single} 
...
Section \ref{sec:assem-discussion-single} verified that maximizing the total fission 
...
Section \ref{sec:assem-discussion-single} verified that the \gls{AHTR} one-third assembly 
...
the a-2a and a-2b double-objective optimization simulations resulting in widely spread out
...
the largest and final optimization problem run for the \gls{AHTR} one-third assembly 
model. 
...
The triple-objective optimization simulations successfully found a wide spread of 
...
simulation. 
One may refer to Table \ref{tab:assem-obj-breakdown} for each simulation's parameters.
\begin{table}[htbp!]
    \centering
    \onehalfspacing
...
a-1a, a-1b, a-1c, a-1d, a-1e, a-1f; and Section \ref{sec:assem-discussion-single}'s
discussions, I verified that each of the one-third assembly objective follows the 
same driving factors as the \gls{AHTR} plank optimization objectives (Chapter 
\ref{chap:ahtr-plank-opt-results}) and described each objective's relationship with 
each input parameter. 
I determined that \gls{ROLLO} flattens TRISO distribution and maximizes the coolant 
channel shape's radius values ($r_1, r_2, r_3, r_4, r_5$) that are located close to the 
...
discussions in Section \ref{sec:assem-discussion-two} and 
\ref{sec:assem-discussion-triple}, I further analyzed how the objectives' combined 
effects resulted in the optimal reactor models found by each multi-objective 
...
the coolant channel shape's radius values ($r_1, r_2, r_3, r_4, r_5$) that are located 
close to the reactor model's temperature peak. 
...
Simulation a-3b is the final and largest multi-objective optimization problem 
that minimized all three objectives ($PF_{total}$, $T_{max}$, and 
$PPF_{fuel}$) while varying all the input parameters ($PF_{total}$, TRISO distribution, 
and coolant channel shape).
Simulation a-3b ran for $\sim 1800$ node-hours on the Theta supercomputer 
with 6 generations and 128 reactor models per generation. 
