...
distributions, labeled numerically, in the final generation that fall on the 
Pareto front. 
\begin{figure}[htbp!]
    \begin{subfigure}{\textwidth}
        \centering
...
the most-minimized $PF_{total}$ is reactor model 30 (the corresponding geometry is illustrated in Figure 
...
In the top right Figure \ref{fig:assem-obj-3-most-minimized-distr}, the one-third 
...
In Figure \ref{fig:assem-obj-3-most-minimized-distr} in the bottom left, the 
...
Simulation a-3b-256 ran for 3359 node-hours on the Theta supercomputer with 6 
generations and 128 reactor models per generation, which is ten times the compute 
time per single-objective simulation, and three times the compute time per 
...
with the most-minimized $PF_{total}$ is reactor model 11 (the corresponding geometry 
is illustrated in Figure \ref{fig:assem-obj-3-all-min-pf}). 
...
In Figure \ref{fig:assem-obj-3-all-most-minimized-distr}, the one-third assembly model 
with the most-minimized $T_{max}$ is reactor model 1 (the corresponding geometry is 
illustrated in Figure \ref{fig:assem-obj-3-all-min-temp}). 
Reactor model 1 has an almost constant TRISO packing fraction distribution with 
a packing fraction standard deviation of $0.001$ across the one-third assembly. 

In Figure \ref{fig:assem-obj-3-all-most-minimized-distr}, the one-third assembly model 
with the most-minimized $PPF_{fuel}$ is reactor model 4 (the corresponding geometry is illustrated in Figure 
...
In Figure \ref{fig:assem-obj-3-all-most-minimized-distr}, the one-third assembly model 
that equally minimized all three objectives is reactor model 2 (the corresponding geometry is illustrated in 
...
However, they have different $PF_{total}$ values, and simulation a-3b has a slightly 
...
% todo: state where the temp peaks are. 
\begin{figure}[htbp!]
    \begin{subfigure}{\textwidth}
        \centering
...
channels (corresponding to $r_1, r_2, r_3, r_4, r_5$) closest to the temperature peaks 
are most important to minimizing $T_{max}$. 
However, coolant channel shape variation does not have as high of an impact on 
$T_{max}$ as \gls{TRISO} distribution variation: the average $T_{max}$ due to 
\gls{TRISO} variation decreased by ${\sim}150K$ over 3 generations, while average 
$T_{max}$ due to coolant channel shape variation only decreased by ${\sim}10K$ over 
3 generations. 
Sections \ref{sec:assem-discussion-pf}, \ref{sec:assem-discussion-temp}, and 
\ref{sec:assem-discussion-ppf} also concluded that only coolant channel shape only 
correlates with the minimize $T_{max}$ objective. 
...
This could be due to the coolant channel shape not having a high impact on $T_{max}$ 
compared to TRISO distribution; thus, \gls{ROLLO} was more influenced by TRISO 
distribution when searching for optimal reactor models. 
...
simulations, which have 7 or fewer input parameters. 
Larger population size will enable \gls{ROLLO} to explore more reactor model 
variations and potentially find even more optimal reactor models. 
To better explore simulation a-3b's design space, I re-ran simulation a-3b with 
...
% you need to relate it back to the original a-3b simulation and also compare 
% it to the single objectives from before, especiallly if you see differences 
% from original a-3b simulation. 
