...
\begin{figure}[htbp!]
    ...
    maximum plank temperature ($T_{max}$) and normalized power peaking factor (PPF) in the plank. 
    Input parameters varied: TRISO distribution. 
    AHTR models and TRISO distribution for the 3 reactor models on the Pareto front
    that most minimized each objective.}
    \label{fig:slab-obj-3-most-minimized}
\end{figure}
The purple distribution with oscillating fuel packing pattern most-minimized total 
packing fraction. 
The orange distribution with a mostly flat fuel distribution most-minimized $T_{max}$. 
The pink distribution with small oscillating fuel packing pattern most-minimized 
