...
In this chapter, I explore optimizing the AHTR slab geometry for three 
objectives, described in the next section. 
Chapter \ref{ahtr_rollo_assembly} will explore optimizing the AHTR one-third 
fuel assembly.
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
In the subsequent sections, I will describe the AHTR slab geometry, how 
I set up and verified each of their the OpenMC and Moltres models, and the 
results of the single and multi objective \gls{AHTR} slab optimization problems. 
...
In this section, I demonstrate Moltres' ability to reproduce key neutronics 
parameters using group constant data from OpenMC for the AHTR slab.  
...
I used group constants produced by OpenMC in the Moltres simulation. 
The Moltres AHTR Steady-State Temperature Model first solves for the neutronics 
in slab, and feeds that into a thermal hydraulics simulation to solve for the 
temperature distribution in the slab for a defined power. 

For the temperature variable, all boundaries are insulated and we simulate salt 
...
Table \ref{tab:heat-exchanger-constants} shows the constant values used for 
reference temperature and heat transfer coefficient.
\begin{table}[H]
    \centering
    \onehalfspacing
...
I calculated the heat transfer coefficient with the assumption that each 1cm 
AHTR dz slice removes the same amount of power. 
\begin{align}
    h &= \frac{P_{slab}}{A_{coolant}} \div \frac{T_{total}}{H} \\
...
% plots of the results for constant and peaking PF. 
% say the shortcomings of this temperature model 
% justify why its ok 
