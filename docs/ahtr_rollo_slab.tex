...
\ref{sec:plank-one-obj}) and multi-objective (Sections \ref{sec:plank-two-obj} and 
\ref{sec:plank-three-obj}) \gls{ROLLO} optimization simulation results, 
report each simulation's computational cost (Section \ref{sec:plank-compute-cost}), 
and discuss the results' significance (Section \ref{sec:plank-discussion}).
...
In the following subsections, I describe each single-objective simulation result 
...
This section describes the single-objective optimization simulations, p-1a and p-1d, 
results for the minimize total fuel packing fraction ($PF_{total}$) objective. 
The minimize $PF_{total}$ objective is important because a reactor that uses less fuel
with similar performance enables cost savings. % TODO am i sure?  
...
another, Section \ref{sec:plank-discussion-pf} discusses the driving factors for
...
Figure \ref{fig:slab-obj-1-pf-final-coolant} shows a plot of the total radius 
($r_{top} + r_{bot}$) against $PF_{total}$.  
I presented total radius because ... 
% TODO Why present this as rtop and rbot? Do you see trends if you hold one constant 
% and vary the other? 
\begin{figure}[htbp!]
    \centering
...
peak tempeature at a specific power enables the system to perform at a higher 
power at lower thermal stress. % TODO am i sure?  
...
(6.62cm and 20.48cm). 
Because it is more productive to compare all of the single objective results to one 
another, Section \ref{sec:plank-discussion-temp} discusses and explains simulation 
...
\begin{table}[htbp!]
    ...
    \footnotesize
    \begin{tabular}{p{1.5cm}lp{3.7cm}p{3.7cm}p{2.7cm}}
    \hline
    \textbf{Energy Group} & 
    \textbf{$\%$ of Total} &
...
between the plank's $T_{max}$ and total radius ($r_{tot} = r_{bot} + r_{top}$), shown in 
...
shows that the coolant channel shape variation does not have as high of an impact on 
...
to the coolant channel shape variation only decreased by 15K over 10 generations. 

\paragraph{Summary}
The minimize $T_{max}$ objective is driven by minimizing the plank's maximum temperature; 
and flattens the TRISO distribution and maximizes the coolant channel shape's 
