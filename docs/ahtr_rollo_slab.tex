...
distribution and a constant $PF_{total}$ = 0.023 TRISO distribution (shown in 
...
In the following Section \ref{sec:plank-discussion-ppf-results}, I demonstrate the 
relationship derived in Section \ref{sec:plank-discussion-ppf-equation}. 
...
\begin{align}
    ...
    max(\Phi_j)/ave(\Phi_j) &\propto max(\frac{fqr_j}{PF_j}) \div ave(\frac{fqr_j}{PF_j}) \nonumber \\
    &\propto PPF_{fuel}
\end{align}

Therefore, from Equation \ref{eq:flux-prop-ppf}, a flatter flux (smaller 
$max(\Phi_j)/ave(\Phi_j)$ value) will result in a smaller $PPF_{fuel}$. 
...
In simulation p-1c, \gls{ROLLO} found that an \gls{AHTR} plank model with a 
TRISO distribution with peaks near the edges of the fuel region of the plank and a
minimum point in center of the plank, most-minimized $PPF_{fuel}$. 

I ran a simulation for constant $PF_{total}$ = 0.00979 and compared its 
flux with the 
