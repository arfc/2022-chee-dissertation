...
$k_{eff} \leq 1.35$ constraint (Figure \ref{fig:slab-obj-1-pf}). 

I ran a simulation for constant $PF_{total}$ = 0.023 and compared its 
flux and fission reaction rate with the oscillating fuel packing pattern TRISO
distribution to understand why the oscillating fuel pattern enabled a lower 
packing fraction. 
...
\begin{figure}[htbp!]
    \centering
    \includegraphics[width=\linewidth]{triso-0.023.png} 
    \caption{}
    \label{fig:triso-0.023}
\end{figure}
Figure \ref{fig:flux-comparison-0.023-plank} compares the flux distributions from 
TRISO distributions in Figure \ref{fig:triso-0.023}. 
\begin{figure}[htbp!]
    \centering
...
In Group 4, the most-minimized $PF_{total}$ TRISO distribution has 1.0018x higher  
\texttt{fission} (total fission reaction rate) than the 
constant $PF_{total}$ = 0.023 TRISO distribution.

Minimize $PF_{total}$ objective is driven by maximizing the fission reaction rates 
in thermal energy group (Group 4). 
