...
in which all objectives are minimized: total fuel packing fraction ($PF_{total}$), 
maximum plank temperature ($T_{max}$) and fuel-normalized power peaking factor 
($PPF_{fuel}$).  
...
total coolant channel radius of 0.505cm 
(illustrated in Figure \ref{fig:slab-obj-3-all-min-pf}).
...
TRISO distribution, $PF_{total} = 0.034$, and total coolant channel radius of 0.657cm 
...
TRISO distribution, $PF_{total} = 0.036$, and total coolant channel radius of 0.307cm 
(illustrated in Figure \ref{fig:slab-obj-3-all-min-ppf}).
Section \ref{sec:plank-discussion-multi} discusses and explains simulation p-3b's results.
