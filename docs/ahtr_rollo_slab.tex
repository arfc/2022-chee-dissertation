...
In Section \ref{sec:one-obj-pf}, I conducted single-objective optimization 
simulations to minimize total fuel packing fraction ($PF_{total}$). 
In simulation p-1a, \gls{ROLLO} found that an \gls{AHTR} plank model with an 
oscillating fuel packing pattern most-minimized $PF_{total}$ while meeting the 
$k_{eff} \geq 1.35$ constraint (Figure \ref{fig:slab-obj-1-pf}). 

I ran a simulation for constant $PF_{total}$ = 0.023 and compared its 
fission reaction rate with the oscillating fuel packing pattern TRISO
distribution to understand why the oscillating TRISO distribution enabled a lower 
packing fraction. 
...
TRISO distribution (oscillating fuel packing pattern) and the constant 
...
\begin{figure}[htbp!]
    ...
    (oscillating fuel packing pattern) from Figure \ref{fig:slab-obj-1-pf} and the 
...
(OpenMC \texttt{fission} tally) between the most-minimized $PF_{total}$ TRISO 
...
\begin{table}[htbp!]
    ...
    \end{tabular}
\end{table}
In Group 4, the most-minimized $PF_{total}$ TRISO distribution has $0.18\%$ higher  
...
Therefore, the minimize $PF_{total}$ objective is driven by maximizing the fission 
reaction rates in the thermal energy group (Group 4). 
...
In Section \ref{sec:one-obj-ppf}, I conducted single-objective optimization 
...
\end{figure}
In Figure \ref{fig:flux-comparison-0.0979-plank}, the constant distribution's Group 4 flux 
dips in the center of the plank due to spatial self-shielding effects. 
...
between the most-minimized $PPF_{fuel}$ TRISO distribution and a constant 
$PF_{total}$ = 0.0979 TRISO distribution (both shown in Figure \ref{fig:triso-0.023}).
\begin{table}[htbp!]
    \centering
    \onehalfspacing
...
In Section \ref{sec:p-2b}, I conducted a two-objective optimization 
simulation to minimize fuel-normalized power peaking factor ($PPF_{fuel}$) and 
...
why the latter enabled a lower $PPF_{fuel}$. 
Figure \ref{fig:triso-0.029} shows the TRISO distributions for the two compared 
simulations: Figure xx's most-minimized $PPF_{fuel}$
...
\begin{figure}[htbp!]
    ...
    from Figure xx and the constant $PF_{total}$ = 0.029 
    TRISO distribution.}
    \label{fig:triso-0.029}
\end{figure}
...
\begin{figure}[htbp!]
    ...
    \caption{Flux comparison between Figure xx's TRISO 
