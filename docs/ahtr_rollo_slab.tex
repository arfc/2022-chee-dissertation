...
Figure \ref{fig:slab-obj-1-pf-final} shows the five $\rho_{TRISO}(\vec{r})$
in the final generation with the most-minimized $PF_{total}$, and 
Figure \ref{fig:slab-obj-1-pf-most-minimized} illustrates the \gls{AHTR} plank model 
with most-minimized $PF_{total}$.
\begin{figure}[htbp!]
    \centering
    ...
    packing fraction. Input parameters varied: total fuel packing fraction 
    ($PF_{total}$), \gls{TRISO} packing fraction distribution.}
...
$PPF_{fuel}$ peaks near the edges of the fuel region of the plank, and has a minimum point in 
center of the plank.
...
\begin{figure}[htbp!]
    ...
    total packing fraction ($PF_{total}$) and normalized power peaking factor ($PPF_{fuel}$) in the plank. 
    Input parameters varied: TRISO distribution.}
\end{figure}

All reactor models on Figure \ref{fig:slab-obj-2-pfppf}'s Pareto front have a relatively flat 
...
This distribution differs from simulation p-1c's most-minimized $PPF_{fuel}$ TRISO distribution 
...
0.0979, while the total PF in the most-minimized $PPF_{fuel}$ model (blue distribution) is 0.029. 
...
\subsubsection{Sensitivity Analysis: $PF_{total}$ and $PPF_{fuel}$}
...
\begin{figure}[htbp!]
    ...
    ($T_{max}$) and normalized power peaking factor ($PPF_{fuel}$) in the plank. 
    Input parameters varied: TRISO distribution.}
...
\begin{figure}[htbp!]
    ...
    maximum plank temperature ($T_{max}$) and normalized power peaking factor ($PPF_{fuel}$) 
    in the plank. 
    Input parameters varied: TRISO distribution.}
\end{figure}
...
\begin{figure}[htbp!]
    ...
    maximum plank temperature ($T_{max}$) and normalized power peaking factor ($PPF_{fuel}$) in the plank. 
...
\subsection{p-3b: Variation of $\rho_{TRISO}(\vec{r})$, Total Packing Fraction, and Coolant channel shape}
...
\begin{figure}[htbp!]
    ...
    \begin{subfigure}{\textwidth}
        ...
        $T_{max}$ and $PPF_{fuel}$ (corresponds to the pink distribution in the above plot).}
