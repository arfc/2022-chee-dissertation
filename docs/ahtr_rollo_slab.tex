...
\subsubsection{Simulation p-1a: Variation of $PF_{total}$ and $\rho_{TRISO}(\vec{r})$}
...
\begin{table}[htbp!]
    ...
    \end{tabular}
\end{table}
Figure \ref{fig:slab-obj-1-pf-evol} shows simulation p-1a's $PF_{total}$ evolution,  
...
The results from simulation p-1b and p-1e demonstrate that the minimize $T_{max}$ 
objective is more influenced by the TRISO distribution than the coolant channel shape. 
...
\section{AHTR Plank: Double-Objective Optimization Results}
...
\section{AHTR Plank: Triple-Objective Optimization Results}
...
out reactor models in the final generation's Pareto front.  
...
Section \ref{sec:plank-discussion-three} discusses and explains simulation p-3a's 
results.
