...
Figure \ref{fig:p-2b-pf-triso-comparison} shows that simulation p-1a's most-minimized 
$PF_{total}$ oscillating TRISO distribution has a high $PPF_{fuel} = 1.363$.
...
with the most-minimized $PPF_{fuel}$ has a TRISO distribution that peaks in the plank's 
center and both sides with a $\sim0.02$ variation, which differs from simulation 
p-1c's most-minimized $PPF_{fuel}$ TRISO distribution which has a large $\sim 0.07$ 
variation and a minimum point in the center of the plank.

I ran a simulation for $PF_{total}$ = 0.029 with simulation p-1c's most-minimized 
$PPF_{fuel}$ TRISO distribution shape and compared its flux to simulation p-2b's 
most-minimized $PPF_{fuel}$ TRISO distribution to understand why the latter enabled a 
lower $PPF_{fuel}$. 
...
$PPF_{fuel}$ TRISO distribution and the simulation p-1c's most-minimized 
$PPF_{fuel}$ TRISO distribution shape $PF_{total}$ = 0.029 TRISO distribution.
\begin{figure}[htbp!]
    \centering
...
\begin{table}[htbp!]
    ...
    \end{tabular}
\end{table}
In energy group 4, the most-minimized $PPF_{fuel}$ flux distribution is $0.28\%$ 
flatter than the constant $PF_{total}$ = 0.0292 flux distribution, resulting in a 
lower $PPF_{fuel}$ (as shown by Equation \ref{eq:flux-prop-ppf}). 
...
This results in the TRISO distribution variation having a stronger effect in neutralizing 
self-shielding. 
...
% TODO: what does this mean for reactor designers... 
...
\subsection{AHTR Plank: Two-Objective Optimization Major Takeaways}
The results from simulation p-2b and investigations in Section 
