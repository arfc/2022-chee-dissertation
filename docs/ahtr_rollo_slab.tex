...
In this chapter, I demonstrated using \gls{ROLLO} to conduct single objective 
\gls{AHTR} optimization for non-conventional geometries and parameters.
...
before a simple first attempt approach is taken for this dissertation. 
...
\gls{ROLLO} drives the evolutionary algorithm optimization process, and 
utilizes both OpenMC and Moltres software to model the AHTR fuel slab to 
calculate the problem's objective and constraint values. 

In this chapter, I explore optimizing the AHTR slab geometry for these three 
objectives. 
...
results of the single and multi objective \gls{AHTR} optimization problems. 
...
\section{AHTR Slab OpenMC and Moltres Models}
The input parameter modeling outlined in the previous section are inputs into 
the OpenMC neutronics model. 
Therefore, there is TRISO level fidelity in the OpenMC neutronics model. 
...
For successful \gls{AHTR} Moltres simulation, I established 
...
\subsection{Moltres Model's Neutronics Performance}
...
Comparison key neutronics parameters for the continuous OpenMC and multigroup Moltres 
simulations determine if the energy and spatial homogenization used are acceptable. 
...
\subsection{AHTR Steady-State Temperature Model}
