...
Section \ref{sec:ahtr-plank-geometry} described the \gls{AHTR} plank geometry.
Section \ref{sec:input-parameter-modeling} detailed how I vary the 
\gls{AHTR} plank's input parameters. 
Sections \ref{sec:ahtr-moltres-hom} and \ref{sec:ahtr_model_verification}
described and verified the \gls{AHTR} plank's OpenMC neutronics and Moltres 
temperature models. 
Section \ref{sec:opt-problem} describeed the optimization objectives and Section 
\ref{sec:ahtr_slab_output} described how I calculated them from the OpenMC and Moltres 
model outputs. 
Section \ref{sec:hyperparameter-studies} described the hyperparameter tuning process 
...
reports each simulation's computational cost, and discusses their results.
...
Thus, I constrainined $k_{eff} \geq 1.35$ to find optimal input parameters that achieve 
...
This section report the \gls{AHTR} plank's \gls{ROLLO} single-objective 
...
Section \ref{sec:rollo-convergence} describes how reactor designers can use 
...
sine distributions are not the same but follow a similar oscillating 
...
\ref{fig:slab-obj-1-ppf-most-minimized} illustrates the \gls{AHTR} plank model with the
...
In Figure \ref{fig:slab-obj-1-ppf-final}, the TRISO distribution with the most-minimized 
$PPF_{fuel}$ peaks near the edges of the fuel region of the plank and has a minimum 
point at the plank's center.
...
minimize plank's maximum temperature ($T_{max}$), I do not conduct two-objective 
...
The \gls{TRISO} distributions on the Pareto front that minimize both $PF_{total}$ and 
...
The \gls{TRISO} distributions on the Pareto front that minimize both $T_{max}$ 
...
has an almost constant packing fraction of $\sim0.0979$ across the plank, with a 
...
Sections \ref{sec:plank-discussion-multi} discusses and explain simulation p-2c's results.
...
\begin{figure}[htbp!]
    ...
    \begin{subfigure}{0.85\textwidth}
        ...
        \caption{TRISO distribution for the reactor models on the Pareto front
...
In Figure \ref{fig:slab-obj-3-distr-most-minimized}, the TRISO distribution with the 
most-minimized $T_{max}$ (the orange distribution) has a mostly flat TRISO distribution 
and $PF_{total} = 0.059$ (also illustrated in Figure \ref{fig:slab-obj-3-min-temp}). 
In Figure \ref{fig:slab-obj-3-distr-most-minimized}, the TRISO distribution with the
...
\begin{figure}[htbp!]
    \begin{subfigure}{\textwidth}
        ...
        \caption{TRISO distribution for the reactor models on the Pareto 
    ...
    \begin{subfigure}{\textwidth}
        ...
        $PF_{total}$ (corresponds to the pink distribution in the above plot).}
        \label{fig:slab-obj-3-all-min-pf}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
        \includegraphics[width=\linewidth]{slab-obj-3-all-min-temp.png}
        \caption{TRISO distribution and coolant channel shape that most minimized 
        $T_{max}$ (corresponds to the dark green distribution in the above plot).}
        \label{fig:slab-obj-3-all-min-temp}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
        \includegraphics[width=\linewidth]{slab-obj-3-all-min-ppf.png}
        \caption{TRISO distribution and coolant channel shape that most minimized 
        $PPF_{fuel}$ (corresponds to the light green distribution in the above plot).}
...
model with the most-minimized $T_{max}$ (dark green distribution) has a flat 
TRISO distribution, $PF_{total} = 0.034$, and $r_{tot}=0.657cm$
(illustrated in Figure \ref{fig:slab-obj-3-all-min-temp}).
In Figure \ref{fig:slab-obj-3-distr-most-minimized-distr-all}, the \gls{AHTR} plank 
model with the most-minimized $PPF_{fuel}$ (light green distribution) has a flat 
TRISO distribution, $PF_{total} = 0.036$, and $r_{tot}=0.307cm$
