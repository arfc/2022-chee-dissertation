...
optimization results. 
I run two three-objective optimization simulations that optimize for all three 
objectives (minimize $PF_{total}$, $T_{max}$, and $PPF_{fuel}$). 
The first varies total fuel packing fraction ($PF_{total}$) and \gls{TRISO} packing 
fraction distribution ($\rho_{TRISO}(\vec{r})$).
The second varies $PF_{total}$ and $\rho_{TRISO}(\vec{r})$, and coolant channel shape. 
