...
attempted, this dissertation attempts a first go at beginning to explore the 
large design space.  
...
I show the pareto front for each multi-objective problem. 
...
constraining $k_{eff} >= 1.35$.
Figure \ref{fig:slab-obj-2-pftemp-pareto} shows a plot of total packing fraction 
against maximum slab temperature for each individual reactor models 
in the ROLLO simulation.
The red crosses refer to the reactor models on final evolutionary algorithm 
generation's pareto front. 
Figure \ref{fig:slab-obj-2-pftemp-pareto-distr} shows the TRISO distribution of 
reactor models on the pareto front. 
The TRISO distribution that corresponds to lowest packing fraction and highest 
maximum slab temperature has an up-down fuel packing pattern which looks similar 
to the TRISO distribution from the minimize packing fraction single objective 
optimization seen in Figure \ref{fig:slab-obj-1-pf-final}.
The TRISO distribution that corresponds to lowest maximum slab temperature and 
highest total packing fraction is mostly flat and looks similar to the 
TRISO distribution from the minimize maximum temperature single objective 
optimization seen in Figure \ref{fig:slab-obj-1-temp-final}.
\begin{figure}[]
    \centering
    \begin{subfigure}{\textwidth}
    ...
    \end{subfigure}
    \caption{ROLLO multi-objective optimization to minimize total packing fraction and 
    maximum temperature in the slab. Input parameters varied: 
    TRISO particle distribution and total packing fraction.}
    \label{fig:slab-obj-2-pftemp}
\end{figure}

\subsubsection{Two Objectives: Minimize Total Packing Fraction and Normalized Power Peaking Factor}
I varied the total packing fraction and TRISO particle distribution while 
constraining $k_{eff} >= 1.35$.
Figure \ref{fig:slab-obj-2-pfppf-pareto} shows a plot of total packing fraction 
against normalized power peaking factor for each individual reactor models 
in the ROLLO simulation.
The red crosses refer to the reactor models on final evolutionary algorithm 
generation's pareto front. 
The pareto front shows a negative correlation between packing fraction and 
PPF. 
Higher PPF results in a lower packing fraction and vice versa. 
Figure \ref{fig:slab-obj-2-pfppf-pareto-distr} shows the TRISO distribution of 
reactor models on the pareto front. 
The TRISO distribution with the highest power peaking factor and lowest packing 
fraction has a constant packing fraction of 0.02 across the slab. 
The TRISO distribution with the lowest power peaking factor and highest packing 
fraction has a packing fraction that peaks in the center of the slab and both 
sides. 
% this is probably due to self shielding phenomena. 
\begin{figure}[]
    \centering
    \begin{subfigure}{\textwidth}
    ...
    \end{subfigure}
    \caption{ROLLO multi-objective optimization to minimize total packing fraction and 
    normalized power peaking factor in the slab. Input parameters varied: 
    TRISO particle distribution and total packing fraction.}
    \label{fig:slab-obj-2-pfppf}
\end{figure}

\subsubsection{Two Objectives: Minimize Maximum Temperature and Normalized Power Peaking Factor}
I varied the TRISO particle distribution while constraining $k_{eff} >= 1.00$
and holding total packing fraction at 0.0979.
Figure \ref{fig:slab-obj-2-tempppf-pareto} shows a plot of maximum temperature 
against normalized power peaking factor for each individual reactor models 
in the ROLLO simulation. 
The black dots represent individual reactor models, and the red crosses refer to 
the reactor models on the pareto front. 
The pareto front shows a negative correlation between maximum temperature and 
PPF. 
Higher PPF results in lower maximum temperature and vice versa. 
Figure \ref{fig:slab-obj-2-tempppf-pareto-distr} shows the TRISO distribution 
for the reactor models on the pareto front. 
The TRISO distribution that corresponds to lowest maximum temperature and highest 
normalized power peaking factor is mostly flat and looks similar to the 
TRISO distribution from the minimize maximum temperature single objective 
optimization described in Section \ref{sec:1-obj-TRISO-temp}. 
The TRISO distribution that corresponds to the highest maximum temperature and 
lowest normalized power peaking factor has high packing fraction on the sides 
and reaches a minimum point at the center of the slab, which looks similar to the 
TRISO distribution from the minimize PPF single objective optimization
described in Section \ref{sec:1-obj-TRISO-ppf}.
TRISO distribution that has an average of each objective has a TRISO distribution 
that alternates between higher and lower packing fraction with a flatter TRISO 
distribution in the center of the slab. 
\begin{figure}[]
    \centering
    \begin{subfigure}{\textwidth}
    ...
    \end{subfigure}
    \caption{ROLLO multi-objective optimization to minimize maximum temperature and 
    normalized power peaking factor in the slab. Input parameters varied: 
    TRISO particle distribution.}
    \label{fig:slab-obj-2-tempppf}
\end{figure}

\subsection{Three Objectives: Minimize Total Packing Fraction, Maximum Temperature, and Normalized Power Peaking Factor}
