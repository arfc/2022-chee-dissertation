...
motivating factors for each individual objective, and how their combined effects 
...
For these optimization problems, \gls{ROLLO} conducted a global search of the large 
design space helping narrow down possible solutions.
The widely spread reactor models on the Pareto front informs reactor designers of 
optimal reactor models for their defined objectives.  
...
section of the design space that they are interested in.
...
parameter. 
I found that the minimize $T_{max}$ objective is driven by minimizing plank's maximum 
...
\ref{sec:plank-discussion-multi} I further analyzed how the objectives' combined effects 
resulted in the optimal reactor models found by each multi-objective optimization 
simulation. 
...
Figure \ref{fig:slab-obj-3-all} shows the 21 widely spread reactor models on simulation 
p-3b's Pareto front. 
Simulation p-3b's results show the set of reactor models that meet the three objectives 
to varying degrees. 
...
models on the Pareto front that satisfy the problem's objectives. 
Reactor designers can utilize \gls{ROLLO} to narrow down the search space for reactor 
models to meet any number of objectives and input parameters. 
...
further characterize a smaller section of the design space that they are interested in.

By characterizing each objective's main driving forces for the \gls{AHTR} plank model ...  
% talk about how characterizing helps with one-third assem
