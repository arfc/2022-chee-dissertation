...
optimization simulation to minimize the fuel-normalized power peaking factor ($PPF_{fuel}$) 
and the total fuel packing fraction ($PF_{total}$) by varying the TRISO distribution. 
...
(the blue solid distribution in Figure \ref{fig:slab-obj-2-pftemp-pareto-distr}). 
...
(the purple dashed distribution in Figure \ref{fig:slab-obj-2-pftemp-pareto-distr}).
...
(the orange dashed distribution in Figure \ref{fig:slab-obj-2-pfppf-pareto-distr}). 
...
with a $\sim0.02$ variation and $PF_{total}=0.029$ (the blue solid distribution in Figure 
\ref{fig:slab-obj-2-pfppf-pareto-distr}). 
This distribution differs from simulation p-1c's most-minimized $PPF_{fuel}$ TRISO 
distribution (Figure \ref{fig:slab-obj-1-ppf-final}) which has a large $\sim 0.07$ 
variation in the TRISO distribution with a minimum point in the center of the plank. 
...
minimize the $PPF_{fuel}$ objective's driving factor flattening thermal flux distribution 
