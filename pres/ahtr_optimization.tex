...
\begin{frame}
    ...
    \begin{columns}[t]
        ...
        \begin{figure}
            \only<2>{\includegraphics[width=\linewidth]{../docs/figures/ahtr_assembly.png} 
...
\begin{frame}
    ...
    \only<1>{
        ...
        \only<2>{
            \begin{figure}
                \includegraphics[width=0.95\linewidth]{figures/ahtr-assem-opt-table-annotated.png}
            \end{figure}}
...
\begin{frame}
    ...
    \only<4>{\begin{tcolorbox}[colback=illiniorange,colframe=illiniorange!50!black]
...
\begin{frame}
    ...
    \only<1>{Black dot: reactor model, red cross: reactor model with lowest $T_{max}$.}
...
\begin{frame}
    ...
    \visible<2->{\textbf{Minimize $PF_{total}$ Objective} 
    \begin{itemize}
    ...
    \begin{itemize}
        \item Minimize $T_{max}$ objective prefers a flatter TRISO distribution 
        \item FLiBe channels located closest to temperature peaks shows a negative 
        correlation with $T_{max}$
    \end{itemize}}

    \vspace{0.2cm}
    \visible<4->{\textbf{Minimize $PPF_{fuel}$ Objective} 
    \begin{itemize}
...
\begin{frame}
    ...
    \textbf{Simulation a-2b: I vary $PF_{total}$ and a, b, c, d, e f
    ($\rho_{TRISO}(\vec{x}, \vec{y}$)) to minimize $PF_{total}$ and $PPF_{fuel}$.} 
