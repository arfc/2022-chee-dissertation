    ...
    \begin{columns}[t]
        ...
        \only<2>{\begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                degrees; this set of solutions is called the Pareto front}
        ...
        \only<3>{\begin{column}{0.35\textwidth}
            \begin{block}{Simulation a-2b}
            \begin{itemize}
            \item 12 reactor models on simulation a-2b's Pareto front have different 
            $PF_{total}$ and $\rho_{TRISO}(\vec{r})$ depending on \textbf{extent each
            objective is minimized.}
            \item Reactor model 11 = lowest $PF_{total}$, highest $PPF_{fuel}$
            \item Reactor model 3 = lowest $PPF_{fuel}$, highest $PF_{total}$
            \item Reactor model 5 = minimizes both objectives to an equal extent 
            \end{itemize}
            \end{block}
            \end{column}}
    \only<1,3>{    
