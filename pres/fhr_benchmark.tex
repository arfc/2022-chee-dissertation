...
\begin{frame}
    ...
    \only<2>{$k_{eff}$: Effective neutron multiplication factor, average number of 
    neutrons from one fission that cause another fission.}
    \only<3>{Reactivity coefficients: how much keff is changing when temperature of the 
    material changes.}
    \begin{table}
