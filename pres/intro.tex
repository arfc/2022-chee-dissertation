\subsection{Overview}
    \begin{frame}
        ...
        \begin{enumerate}
            \only<1>{\item Furthered our understanding of the \gls{AHTR} design's complexities 
            through neutronics and temperature modeling}
            \only<2>{\item Created an open-source tool that enables reactor generative 
            design optimization with evolutionary algorithms}
            \only<3>{\item Applied the optimization tool to the \gls{AHTR} design for 
