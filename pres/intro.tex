...
\subsection{Background: Advanced High Temperature Reactor}
    \begin{frame}
        ...
        \vspace{-0.1cm}
        The \textbf{Fluoride-Salt Cooled High-Temperature Reactor (FHR) system 
    ...
    assemblies within the core, and TRISO particles embedded in stripes within each plank.}
    \end{frame}
...
\begin{frame}
    ...
    \visible<1->{
    With further advancement of additive manufacturing technologies, a reactor core 
    could be 3D printed within the next decade.
    ...
    \visible<2->{\begin{figure}[]
        \begin{minipage}[r]{0.45\textwidth}
            \includegraphics[width=0.6\linewidth]{figures/tungsten-wavy-channels.png}            
        \end{minipage}
        \begin{minipage}[l]{0.5\textwidth}
            \caption{3D printed Tungsten wavy flow channels, reproduced from NASA's 
            Refractory Alloy Additive Manufacturing Build Optimization Project (RAAMBO) 
            \cite{mireles_refractory_2022}.}        
        \end{minipage}
    \end{figure}}

    \visible<3->{3D printing for reactor parts could \textbf{
    reduce fabrication costs and deployment timelines, and improve reactor safety and 
    performance}.}

    \vspace{0.1cm}
    \visible<4->{Due to the expansion of the potential design space enabled by 
    3D printing, we need to find methods, such as \textbf{generative design}, to 
